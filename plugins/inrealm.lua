...
local function create_realm(msg)
        ...
        if is_sudo(msg) or is_realm(msg) and is_admin(msg) then
                local group_creator = msg.from.print_name
                create_group_chat (group_creator, group_name, ok_cb, false)
                return 'مقر فرماندهی [ '..string.gsub(group_name, '_', ' ')..' ] ساخته شد☻.'
        end
end
...
local function get_group_type(msg)
  ...
  if data[tostring(msg.to.id)] then
    if not data[tostring(msg.to.id)]['group_type'] then
     return 'گروه عنوان ندارد.'
    end
     local group_type = data[tostring(msg.to.id)]['group_type']
     return group_type
  else 
     return 'عنوان گروه پیدا نشد.'
  end 
end
...
local function set_description(msg, data, target, about)
    if not is_admin(msg) then
        return "برای ادمین ها میباشد."
    end
    local data_cat = 'description'
        data[tostring(target)][data_cat] = about
        save_data(_config.moderation.data, data)
        return 'مشاخصات گروه شما:\n'..about
end
...
local function lock_group_member(msg, data, target)
    ...
    local group_member_lock = data[tostring(target)]['settings']['lock_member']
        if group_member_lock == 'yes' then
            return 'اعضای گروه هم اکنون قفل هستند'
        else
            data[tostring(target)]['settings']['lock_member'] = 'yes'
            save_data(_config.moderation.data, data)
        end
        return 'اعضای گروه قفل شدن'
end
...
local function lock_group_flood(msg, data, target)
    ...
    local group_flood_lock = data[tostring(target)]['settings']['flood']
        if group_flood_lock == 'yes' then
            return 'مقدار اسپم بسته است'
        else
            data[tostring(target)]['settings']['flood'] = 'yes'
            save_data(_config.moderation.data, data)
        return 'مقدار اسپم بسته شد'
        end
end
...
local function unlock_group_flood(msg, data, target)
    ...
    local group_flood_lock = data[tostring(target)]['settings']['flood']
        if group_flood_lock == 'no' then
            return 'مقدار اسپم بسته نمیباشد'
        else
            data[tostring(target)]['settings']['flood'] = 'no'
            save_data(_config.moderation.data, data)
        return 'مقدار اسپم بازشد'
        end
end
-- show group settings
...
local function admin_promote(msg, admin_id)
        ...
        if data[tostring(admins)][tostring(admin_id)] then
                return admin_name..' هم اکنون یک ادمین است'
        end
...
local function admin_demote(msg, admin_id)
    ...
    local data = load_data(_config.moderation.data)
        ...
        if not data[tostring(admins)][tostring(admin_id)] then
                return admin_id..' ادمین نمیباشد'
        end
        data[tostring(admins)][tostring(admin_id)] = nil
        save_data(_config.moderation.data, data)
        return admin_id..' از مقامش سقوط کرد'
end
...
local function admin_user_promote(receiver, member_username, member_id)
        ...
        if data['admins'][tostring(member_id)] then
                return send_large_msg(receiver, member_username..' او هم اکنون یک ادمین است'
        end
        data['admins'][tostring(member_id)] = member_username
        save_data(_config.moderation.data, data)
        return send_large_msg(receiver, '@'..member_username..' به مقام مدیریت ارتقا یافت'
end
...
local function admin_user_demote(receiver, member_username, member_id)
    local data = load_data(_config.moderation.data)
        ...
        if not data['admins'][tostring(member_id)] then
                return send_large_msg(receiver, member_username..' او ادمین نمیباشد'
        end
        data['admins'][tostring(member_id)] = nil
        save_data(_config.moderation.data, data)
        return send_large_msg(receiver, 'Admin '..member_username..' از مقامش سقوط کرد'
end
...
local function username_id(cb_extra, success, result)
   ...
   local text = 'فردی با ایدی @'..member..' در گروه نمیباشد.'
   for k,v in pairs(result.members) do
